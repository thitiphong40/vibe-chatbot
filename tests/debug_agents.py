...
def debug_agent_creation():
    ...
    if os.path.exists(vector_stores_dir):
        chroma_dirs = glob.glob(os.path.join(vector_stores_dir, "chroma_db_*"))
        print(f"   Found {len(chroma_dirs)} vector store directories:")
        for chroma_dir in chroma_dirs:
            print(f"   - {os.path.basename(chroma_dir)}")
    else:
        print(f"   Vector stores directory '{vector_stores_dir}' not found")
    print()
    ...
    for name, info in agents_info.items():
        ...
        print(f"     Vector store: {os.path.basename(info['vectorstore_path'])}")
    print()
    ...
    if orphaned_stores:
        ...
        for store in orphaned_stores:
            print(f"   - {os.path.basename(store)}")
    ...
    print(f"Vector stores: {len(chroma_dirs) if os.path.exists(vector_stores_dir) else 0}")
